...
const videoLink = 'https://www.youtube.com/watch?v=a1mcS4tKGNg'
...
const requestMOvie = async(url,link) =>{
    ...
    if(response.status === 200 || response2.status ===200){
        ...
        document.querySelector('#trailer').addEventListener('click',playTrailer());
    }
...
const displayData = (data,data2) =>{
    ...
    <p class="genres">Genres: ${results.genres[0].name}, ${results.genres[1].name}, ${results.genres[2].name}. <button id="trailer">PLAY TRAILER</button></p>
...
const playTrailer = ()=>{
    window.open('https://www.youtube.com/watch?v=a1mcS4tKGNg','_blank');
}
